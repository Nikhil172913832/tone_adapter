...
export function analyzeVocabularyLevel(texts) {
...
export function analyzeSentenceComplexity(texts) {
    ...
    if (complexSentenceRate > 0.3)
        return 'complex';
    else if (complexSentenceRate < 0.1)
        return 'simple';
    else if (stdDevTokens > avgTokens * 0.5)
        return 'varied';
    else
        return 'compound';
}
export function analyzeDiscourseMarkers(texts, totalMsgs) {
    const interjectionCount = countMatches(texts, INTERJECTION_RE);
    const interjectionRate = interjectionCount / totalMsgs;
    if (interjectionRate > 0.2)
        return 'excessive';
    else if (interjectionRate > 0.1)
        return 'frequent';
    else if (interjectionRate < 0.05)
        return 'minimal';
    else
        return 'moderate';
}
